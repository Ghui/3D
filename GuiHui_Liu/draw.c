...
void add_sphere( struct matrix * points, 
		 ...
		 double step ) {
  int i;
  struct matrix * temp = new_matrix(4,10);
...
void generate_sphere( struct matrix * points, 
  ...
  for(p = 0; p<=1;p +=step){
    for(c = 0; c <= 1; c += step){
      x = r*cos(2*M_PI*c)+cx;
      y = r*sin(2*M_PI * c)*cos(M_PI * p) + cy;
      z = r * sin(2*M_PI * c) * sin(M_PI * p);
...
void generate_torus( struct matrix * points, 
  ...
  for(p = 0; p <= 1; p+=step){
    for(c = 0; c <= 1; c+= step){
      x = (r1*cos(2*M_PI * c)+r2) * cos(2 * M_PI * p)  + cx;
      y = r1 * sin(2 * M_PI * c) + cy;
      z = -sin(2 * M_PI * p) * (r1 * cos(2 * M_PI * c) + r2);
      add_point(points,x,y,z);
    }
  }
}
